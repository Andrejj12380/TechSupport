...
const KnowledgeBase: React.FC<KnowledgeBaseProps> = ({ user }) => {
    ...
    return (
        <div className="flex h-[calc(100vh-140px)] gap-6 antialiased">
            {/* Left Sidebar - List */}
            <div className={`${selectedArticle || isEditing ? 'hidden lg:flex' : 'flex'} w-full lg:w-80 flex-col glass-card rounded-2xl shadow-sm border border-white/10 overflow-hidden glass-surface`}>
                <div className="p-5 border-b border-slate-100 space-y-4">
                    <div className="flex justify-between items-center">
                        <h2 className="text-lg font-bold text-slate-800 text-white flex items-center gap-2">
                        ...
                        {isEngineer && (
                            <button
                                ...
                                className="p-1.5 bg-orange-500/20 text-orange-600 dark:bg-orange-500/30 dark:text-orange-400 rounded-lg hover:bg-orange-500 hover:text-white transition-all"
                    ...
                    <form onSubmit={handleSearch} className="relative group">
                        <input
                            ...
                            className="w-full bg-white/5/60 border border-slate-200 dark:border-slate-600 rounded-xl py-2.5 pl-10 pr-4 text-sm text-white placeholder:text-slate-400 dark:placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary transition-all shadow-inner"
                        />
                        <Search className="absolute left-3.5 top-3 w-4 h-4 text-slate-400 text-white/50 group-focus-within:text-primary transition-colors" />
                    </form>
            ...
            {/* Main Content Area */}
            <div className={`flex-1 flex flex-col glass-card rounded-2xl shadow-sm border border-white/10 overflow-hidden glass-surface ${!selectedArticle && !isEditing ? 'hidden lg:flex' : 'flex'}`}>
                {!selectedArticle && !isEditing ? (
                    <div className="flex-1 flex flex-col items-center justify-center text-white/40 p-12 text-center">
            ...
            {deleteModalOpen && (
                <div className="fixed inset-0 z-[110] flex items-center justify-center p-4" role="dialog" aria-modal="true">
                    ...
                    <div className="glass-card rounded-3xl shadow-2xl w-full max-w-md relative z-10 border border-white/10 p-8 animate-in zoom-in-95 duration-200">
                        <div className="w-16 h-16 bg-red-500/10 rounded-2xl flex items-center justify-center mx-auto mb-6">
                            <AlertTriangle className="w-8 h-8 text-red-500" />
                        </div>
                        <h3 className="text-xl font-bold text-slate-800 text-white text-center mb-2">Удалить статью?</h3>
                        ...
                        <div className="flex gap-3">
                            <button
                                ...
                                className="flex-1 py-3 px-4 bg-white/10 text-slate-600 text-white/70 rounded-xl font-bold hover:bg-slate-200 dark:hover:bg-slate-600 transition-all"
                            >
                                Отмена
                            </button>
                            <button
