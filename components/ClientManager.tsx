...
import { Client, Site, ProductionLine, Equipment, RemoteAccess, Instruction, EquipmentStatus, User } from '../types';
import { IconChevronRight } from './Icons';
import ExcelImportModal from './ExcelImportModal';

const inputClass = "w-full border border-slate-200 rounded-xl p-3 text-sm bg-[#F8FAFC] focus:bg-white focus:ring-2 focus:ring-[#FF5B00]/10 focus:border-[#FF5B00] outline-none transition-all";
...
const Modal = ({ title, children, onClose, onSubmit }: { title: string; children?: React.ReactNode; onClose: () => void; onSubmit: (e: React.FormEvent<HTMLFormElement>) => void }) => (
  <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in duration-150">
      ...
      <form onSubmit={onSubmit} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto custom-scrollbar">
        ...
        <div className="flex gap-3 pt-6 border-t mt-4">
          <button type="button" onClick={onClose} className="flex-1 px-4 py-2.5 border border-slate-200 rounded-xl text-sm font-semibold hover:bg-slate-50 transition-colors">Отмена</button>
...
const getLineStatus = (line: ProductionLine) => {
  ...
  if (warrantyStart && warrantyEnd && now >= warrantyStart && now <= warrantyEnd) {
    ...
    return {
      ...
      remaining,
      tooltip: `Гарантия до ${warrantyEnd.toLocaleDateString()} (${remaining})`
    };
  }
...
const ClientManager: React.FC<ClientManagerProps> = ({ user }) => {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 h-full min-h-[600px]">
      ...
      <div className="lg:w-80 bg-white p-3 rounded-xl border border-slate-200 overflow-y-auto shrink-0 flex flex-col shadow-sm">
        <div className="flex items-center justify-between mb-4 px-2">
          <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Реестр Клиентов</h2>
        ...
        <div className="space-y-1 flex-1">
          {clients.map(c => (
            <div key={c.id} className="group/item">
              <div className={`flex items-center rounded-lg transition-all ${selectedClient?.id === c.id ? 'bg-orange-50/50' : 'hover:bg-slate-50'}`}>
                <button
                  onClick={() => handleClientSelect(c)}
                  className={`flex-1 text-left p-2.5 flex items-center justify-between ${selectedClient?.id === c.id ? 'text-[#FF5B00] font-bold' : 'text-slate-700'}`}
                >
                  <div className="flex items-center gap-2 overflow-hidden">
                    <span className="text-sm truncate">{c.name}</span>
                  </div>
                  <IconChevronRight className={`w-3 h-3 transition-transform ${selectedClient?.id === c.id ? 'rotate-90 text-[#FF5B00]' : 'text-slate-300'}`} />
                </button>
                {!isViewer && <button onClick={() => setModal({ type: 'client', data: c })} className="p-2 opacity-0 group-hover/item:opacity-100 text-slate-400 hover:text-[#FF5B00] transition-opacity">✎</button>}
                <button onClick={() => handleDeleteClient(c.id)} className="p-2 opacity-0 group-hover/item:opacity-100 text-slate-400 hover:text-red-500 transition-opacity">🗑</button>
              </div>

              {selectedClient?.id === c.id && (
                <div className="ml-4 mt-1 space-y-0.5 border-l-2 pl-2 border-slate-100">
                  {sites.map(s => (
                    <div key={s.id} className="group/site">
                      <div className={`flex items-center rounded ${selectedSite?.id === s.id ? 'bg-slate-100' : ''}`}>
                        <button
                          onClick={() => handleSiteSelect(s)}
                          className={`flex-1 text-left p-2 text-xs flex items-center justify-between ${selectedSite?.id === s.id ? 'text-slate-900 font-bold' : 'text-slate-500 hover:text-[#FF5B00]'}`}
                        >
                          <span className="truncate">{s.name}</span>
                          <IconChevronRight className={`w-2.5 h-2.5 ${selectedSite?.id === s.id ? 'rotate-90' : ''}`} />
                        </button>
                        {!isViewer && <button onClick={() => setModal({ type: 'site', data: s })} className="p-1.5 opacity-0 group-hover/site:opacity-100 text-slate-400 hover:text-[#FF5B00] text-[10px] transition-opacity">✎</button>}
                        <button onClick={() => handleDeleteSite(s.id)} className="p-1.5 opacity-0 group-hover/site:opacity-100 text-slate-400 hover:text-red-500 text-[10px] transition-opacity">🗑</button>
                      </div>

                      {selectedSite?.id === s.id && (
                        <div className="ml-3 mt-1 space-y-0.5 border-l pl-2 border-slate-200">
                          {lines.map(l => (
                            <div key={l.id} className="group/line flex items-center">
                              <button
                                onClick={() => handleLineSelect(l)}
                                className={`flex-1 text-left p-1.5 text-[11px] rounded transition-colors ${selectedLine?.id === l.id ? 'text-[#FF5B00] font-bold bg-white shadow-sm' : 'text-slate-400 hover:bg-slate-50'}`}
                              >
                                {l.name}
                                <span
                                  title={getLineStatus(l).tooltip}
                                  className={`w-1.5 h-1.5 rounded-full ml-2 ${['paid', 'warranty'].includes(getLineStatus(l).status) ? 'bg-emerald-400' : getLineStatus(l).status === 'expired' ? 'bg-red-400' : 'bg-slate-200'}`}
                                ></span>
                              </button>
                              {!isViewer && <button onClick={() => setModal({ type: 'line', data: l })} className="p-1 opacity-0 group-hover/line:opacity-100 text-slate-300 hover:text-[#FF5B00] text-[10px] transition-opacity">✎</button>}
                              <button onClick={() => handleDeleteLine(l.id)} className="p-1 opacity-0 group-hover/line:opacity-100 text-slate-300 hover:text-red-500 text-[10px] transition-opacity">🗑</button>
                            </div>
                          ))}
                          {isEngineer && !isViewer && (
      ...
      {/* Main Content Area */}
      <div className="lg:flex-1 bg-white p-6 rounded-xl border border-slate-200 shadow-sm overflow-y-auto">
        {!selectedLine ? (
          !selectedClient ? (
            ...
            <div className="space-y-8 animate-in fade-in zoom-in duration-300">
              <div>
                <h1 className="text-3xl font-black text-slate-900">{selectedSite.name}</h1>
                <p className="text-slate-400 font-medium">{selectedClient.name} • Площадка</p>
              </div>

              <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                <h3 className="text-sm font-black uppercase tracking-widest text-[#FF5B00] mb-4">Информация о площадке</h3>
                <div className="space-y-4">
                  <div>
                    <div className="text-[10px] text-slate-400 uppercase font-black tracking-widest mb-1">Адрес</div>
                    <p className="text-sm text-slate-700">{selectedSite.address || 'Не указано'}</p>
                  </div>
                  <div>
                    <div className="text-[10px] text-slate-400 uppercase font-black tracking-widest mb-1">Комментарии</div>
                    <p className="text-sm text-slate-700 italic">{selectedSite.notes || 'Нет комментариев'}</p>
                  </div>
                  <div className="flex gap-2">
                    {isEngineer && (
                      <button onClick={() => setModal({ type: 'site', data: selectedSite })} className="px-3 py-1.5 bg-slate-100 rounded-lg text-xs font-bold text-slate-600 hover:bg-slate-200">Редактировать</button>
                    )}
                    {isAdmin && (
                      <button onClick={async () => {
                        if (confirm('Удалить площадку и все связанные данные?')) {
                          // Note: deleteSite not implemented in previous turns, but following the logic
                          // await api.deleteSite(selectedSite.id); 
                          alert('Функция удаления площадки в разработке');
                        }
                      }} className="px-3 py-1.5 bg-red-50 rounded-lg text-xs font-bold text-red-600 hover:bg-red-100">Удалить</button>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div>
                <h3 className="text-sm font-black uppercase tracking-widest text-slate-400 mb-4">Производственные линии ({lines.length})</h3>
                {lines.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {lines.map(line => (
                      <button
                        ...
                        className="p-6 bg-gradient-to-br from-slate-50 to-white rounded-2xl border-2 border-slate-100 hover:border-[#FF5B00] hover:shadow-lg transition-all text-left group"
                      >
                        <div className="flex items-start justify-between mb-3">
                          <div className="w-12 h-12 bg-[#FF5B00]/10 rounded-xl flex items-center justify-center group-hover:bg-[#FF5B00] transition-colors">
                          ...
                          <div className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Линия</div>
                        </div>
                        <h4 className="font-bold text-slate-900 mb-2 group-hover:text-[#FF5B00] transition-colors flex items-center gap-2">
                          {line.name}
                          <span
                            title={getLineStatus(line).tooltip}
                            className={`w-2 h-2 rounded-full ${['paid', 'warranty'].includes(getLineStatus(line).status) ? 'bg-emerald-400' : getLineStatus(line).status === 'expired' ? 'bg-red-400' : 'bg-slate-200'}`}
                          ></span>
                        </h4>
                        <p className="text-xs text-slate-500 line-clamp-2">{line.description || 'Нет описания'}</p>
                      </button>
                    ))}
                  </div>
                ) : (
            ...
            <div className="space-y-8 animate-in fade-in zoom-in duration-300">
              <div>
                <div className="flex items-start justify-between">
                  <div>
                    <h1 className="text-3xl font-black text-slate-900">{selectedClient.name}</h1>
                    <div className="flex items-center gap-3 mt-1">
                      <p className="text-slate-400 font-medium">Карточка клиента</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden">
                  ...
                  <h3 className="text-sm font-black uppercase tracking-widest text-[#FF5B00] mb-4">Информация</h3>
                  <div className="space-y-4 relative z-10">
                    <div>
                      <div className="text-[10px] text-slate-400 uppercase font-black tracking-widest mb-1">Контакты</div>
                      <p className="text-sm text-slate-700 whitespace-pre-wrap">{selectedClient.contact_info || 'Не указано'}</p>
                    </div>
                    <div className="flex gap-2">
                      {isEngineer && (
                        <button onClick={() => setModal({ type: 'client', data: selectedClient })} className="px-3 py-1.5 bg-slate-100 rounded-lg text-xs font-bold text-slate-600 hover:bg-slate-200">Редактировать</button>
                      )}
                      {isAdmin && (
                        <button onClick={async () => {
                ...
                <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                  <h3 className="text-sm font-black uppercase tracking-widest text-slate-400 mb-4">Статистика</h3>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 bg-white rounded-2xl border border-slate-100 text-center">
                      <div className="text-2xl font-black text-slate-800">{sites.length}</div>
                      <div className="text-[10px] uppercase font-bold text-slate-400">Площадок</div>
                    </div>
                    <div className="p-4 bg-white rounded-2xl border border-slate-100 text-center">
                      <div className="text-2xl font-black text-slate-800">{lines.length}</div>
                      <div className="text-[10px] uppercase font-bold text-slate-400">Линий</div>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div className="border-b pb-6">
              <div className="flex justify-between items-start">
                <div>
                  <h1 title={selectedLine.tooltip_message || ''} className="text-3xl font-black text-slate-900 mb-1 cursor-help underline decoration-dotted decoration-slate-200 underline-offset-4 hover:decoration-[#FF5B00] transition-all">
                    {selectedLine.name}
                  </h1>
                  <p className="text-sm text-slate-400 font-medium">{selectedClient?.name} / {selectedSite?.name}</p>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  {isEngineer && !isViewer && (
                    <button
                      ...
                      className="px-4 py-2 bg-white border border-slate-200 text-slate-700 rounded-xl text-xs font-bold hover:border-[#FF5B00] hover:text-[#FF5B00] transition-colors"
                      title="Создать копию линии со всем оборудованием (S/N сбросятся)"
                    >
                      Дублировать
                    </button>
                  )}
                  <button onClick={() => setModal({ type: 'line', data: selectedLine })} className="px-4 py-2 bg-slate-100 text-slate-600 rounded-xl text-xs font-bold hover:bg-slate-200 transition-colors">Настроить линию</button>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 group relative">
                  <h4 className="text-[10px] font-black text-slate-400 uppercase mb-2 tracking-widest">Монтажные Особенности</h4>
                  <p className="text-sm text-slate-700 leading-relaxed italic">{selectedLine.mounting_features || 'Не заполнено'}</p>
                </div>
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 group relative">
                  <h4 className="text-[10px] font-black text-slate-400 uppercase mb-2 tracking-widest">Специфика Эксплуатации</h4>
                  <p className="text-sm text-slate-700 leading-relaxed italic">{selectedLine.operational_specifics || 'Не заполнено'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="md:col-span-2 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden group">
                <div className="absolute top-0 right-0 w-32 h-32 bg-orange-50 rounded-bl-full -mr-16 -mt-16 opacity-50 group-hover:scale-110 transition-transform"></div>
                ...
                {selectedLine.db_ip ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4 relative z-10">
                    <div>
                      <div className="text-[10px] text-slate-400 uppercase font-black tracking-widest mb-1">IP Адрес Сервера</div>
                      <div className="font-mono text-sm text-slate-900 bg-slate-50 px-3 py-2 rounded-xl border border-slate-100">{selectedLine.db_ip}</div>
                    </div>
                    <div>
                      <div className="text-[10px] text-slate-400 uppercase font-black tracking-widest mb-1">Имя Базы</div>
                      <div className="font-bold text-sm text-slate-900 bg-slate-50 px-3 py-2 rounded-xl border border-slate-100">{selectedLine.db_name}</div>
                    </div>
                    <div>
                      <div className="text-[10px] text-slate-400 uppercase font-black tracking-widest mb-1">Логин</div>
                      <div className="text-sm text-slate-700">{selectedLine.db_user}</div>
                    </div>
                    <div>
                      <div className="text-[10px] text-slate-400 uppercase font-black tracking-widest mb-1">Пароль</div>
                      <div className="text-sm text-slate-700 font-mono">••••••••</div>
                    </div>
                    {selectedLine.db_notes && (
                      <div className="sm:col-span-2">
                        <div className="text-[10px] text-slate-400 uppercase font-black tracking-widest mb-1">Комментарий</div>
                        <div className="text-xs text-slate-500 italic">{selectedLine.db_notes}</div>
                      </div>
                    )}
                  </div>
                ) : (
              ...
              {/* Quick Remote Access Info */}
              <div className="bg-slate-900 p-6 rounded-3xl text-white shadow-xl flex flex-col justify-between">
                <div>
                  <h3 className="text-[10px] font-black uppercase tracking-widest text-[#FF5B00] mb-4">Удаленное подключение</h3>
                  {remote.length > 0 ? (
                    <div className="space-y-3">
                      <div className="text-xl font-black">{remote[0].url_or_address}</div>
                      <div className="inline-block px-2 py-1 rounded bg-white/10 text-[10px] font-bold uppercase">{remote[0].type}</div>
                    </div>
                  ) : <div className="text-white/20 text-xs italic">Не настроено</div>}
                </div>
                <button onClick={() => setModal({ type: 'remote', data: remote[0] || null })} className="mt-6 text-[10px] font-bold text-white/40 hover:text-white uppercase tracking-widest underline transition-colors">Все доступы</button>
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setModal({ type: 'import_line', data: null })} className="bg-slate-100 text-slate-600 px-4 py-2 rounded-xl text-xs font-bold hover:bg-slate-200 transition-all flex items-center gap-2">
                    <span>📥</span> Импорт
                  </button>
                    ...
                    {equipment.length > 0 ? equipment.map(e => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <div className="font-bold text-slate-900">{e.model}</div>
                          <div className="text-[10px] font-mono text-slate-400">{e.article || 'Артикул отсутствует'}</div>
                        </td>
                        <td className="px-6 py-4">
                          {e.ip_address ? (
                            <div className="flex flex-col">
                              <span className="font-mono text-slate-700 text-xs">{e.ip_address}</span>
                              <span className="text-[9px] text-slate-400">Mask: {e.subnet_mask || '—'}</span>
                            </div>
                          ) : (
                            <span className="text-slate-300 text-xs italic">не настроено</span>
                          )}
                        </td>
                        <td className="px-6 py-4 text-center">
                          <select
                            ...
                            className={`text-[10px] font-bold uppercase py-1 px-2 rounded-lg focus:outline-none cursor-pointer border border-transparent hover:border-slate-200 transition-all ${e.status === 'active' ? 'bg-green-100 text-green-700' :
                              e.status === 'maintenance' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'
                              }`}
                          >
                        ...
                        <td className="px-6 py-4 text-slate-500 text-xs italic max-w-[200px] truncate">{e.notes}</td>
                        <td className="px-6 py-4 text-right space-x-2">
                          {!isViewer && <button onClick={() => setModal({ type: 'equipment', data: e })} className="p-2 text-slate-300 hover:text-[#FF5B00] transition-colors">✎</button>}
                          {isEngineer && <button onClick={() => handleDeleteEquip(e.id)} className="p-2 text-slate-200 hover:text-red-500 transition-colors">✕</button>}
                        </td>
                      </tr>
                    )) : (
                      <tr><td colSpan={5} className="px-6 py-12 text-center text-slate-300 italic text-sm">Оборудование не добавлено</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
            ...
            >
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-sm font-black uppercase tracking-widest text-slate-400">Документация и Ресурсы</h3>
              ...
              {dragActiveDocs && (
                <div className="absolute inset-0 bg-[#FF5B00]/10 flex items-center justify-center rounded-2xl z-40 pointer-events-none">
                  <div className="bg-white/90 px-6 py-4 rounded-lg text-[#FF5B00] font-bold">Отпустите файлы, чтобы загрузить</div>
                </div>
              )}

              {isUploadingDocs && (
                <div className="absolute top-4 right-4 z-50 bg-white/90 px-3 py-2 rounded-lg text-sm font-semibold">Загрузка...</div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {instructions.length > 0 ? (
                  <>
                    {instructions.map(i => (
                      <div key={i.id} className="group flex items-center p-4 rounded-2xl bg-[#F8FAFC] border border-slate-50 hover:bg-orange-50 hover:border-orange-100 transition-all">
                        <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-lg mr-4 shadow-sm text-[#FF5B00]">📄</div>
                        <div className="flex-1 overflow-hidden">
                          <a
                            ...
                            className="text-xs font-black text-slate-800 hover:underline block truncate"
                            title={i.link}
                          >
                            {i.module_type || 'Файл'}
                          </a>
                          <p className="text-[10px] text-slate-400">Версия {i.version || '1.0'} • {i.notes || 'Без описания'}</p>
                          {copiedLinkId === i.id && (
                            <div className="text-[10px] text-emerald-600 font-bold mt-1">Ссылка скопирована в буфер обмена</div>
                          )}
                        </div>
                        <div className="flex items-center gap-2">
      ...
      {modal?.type === 'client' && (
        ...
        >
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Контакты / Договор</label>
              <textarea name="contact_info" defaultValue={modal.data?.contact_info} className={inputClass} style={{ minHeight: '100px' }} placeholder="ФИО, телефон, номер договора..." />
            </div>
      ...
      {modal?.type === 'line' && (
        ...
        >
          <div className="space-y-6">
            ...
            <div className="pt-4 border-t border-slate-100 space-y-4">
              <div>
                <h4 className="text-[11px] font-black text-emerald-600 uppercase mb-2 tracking-widest">Гарантия (12 мес.)</h4>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
