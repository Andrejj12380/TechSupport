...
const UserAvatar: React.FC<UserAvatarProps> = ({ username, avatarUrl, size = 'md', className = '' }) => {
    const sizeClasses = {
    ...
    // If avatar_url exists, show the uploaded image
    if (avatarUrl) {
        return (
            <img
                src={avatarUrl}
                alt={username}
                title={username}
                className={`rounded-full object-cover shadow-inner ${sizeClasses[size]} ${className}`}
            />
        );
    }

    // Otherwise, generate a deterministic color based on username
    const colors = [
